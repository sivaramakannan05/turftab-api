        ...
        foreach ($config as $name => $logger)
        {
            ...
            if (class_exists($logger_class_name))
            {
                ...
                if (!is_null(@constant('self::' . $logger['level'])))
                {
                    ...
                    $logger_object->set_prefix($logger['prefix']);
                    $logger_object->set_extension($logger['extension']);
...
class FileLogger extends Logger
{
    ...
    {
        $this->_file_name = $this->get_prefix() . $this->get_name() . '-' . date('Y-m-d') . '.'.$this->get_extension();
    }
}
